...
项目经理负责确保采取纠正行动。变更请求带来的是变更，而不是纠正行动。但是，纠正行动可能会带来变更请求。
...
![73506300_1646629489](https://user-images.githubusercontent.com/8033144/173739267-6d5635ed-8e94-4632-a5e2-ac82941eb696.png)
...
![over](https://user-images.githubusercontent.com/8033144/173739465-8a8abd53-8be1-4358-8b21-68eaf464e801.png)
...
- 资源优化：  
  ...
  - 资源平滑：解决预定资源限制的问题，处理结果不延长工期（尽量减小资源负荷的变化，资源线条波动小）  
...
按时间段划分的完工预算（BAC——Budget at Completion）  
完工估算 EAC Estimate   
完工差异 VAC  
完工差异 = 完工预算 - 完工估算  
VAC = BAC - EAC  
...
- EAC = AC + BAC - EV = 当前的实际成本 + 剩余工作的原预算（非典型偏差）  
- EAC= BAC/积累CPI = 当前的实际成本 + 剩余工作的预算/CPI (典型偏差)  
- EAC = 当前的实际成本 + 剩余工作的预算/(CPI * SPI) （典型偏差）  
...
在团队章程中定义的基本规则规定了项目团队成员以及其他相关方在相关方参与方面的预期行为。
...
- 合同  
- 订购单  
- 协议备忘录（MOA, Memorandum of agreement）  
- 服务水平协议（SLA）  
...
- 总价合同（固定价格合同）  
  - 固定总价FFP (买方最喜欢) 
  - 总价加激励费用FPIF （和成本+激励会有个总价关键字）  
  - 总价加经济价格调整FPEPA （一般是合同跨度数年，用于抵消通货膨胀等） 
- 成本补偿合同（成本返还合同）  
  - 成本 + 固定费用合同 （需要很多审计工作，即要审核很多卖方的报销发票）  
  - 成本 + 激励费用合同（有比例）  
  - 成本 + 奖励费用合同    
- 工料合同T&M（单价合同，无法说明工作范围）  
...
### 零碎汇总  
各个分解结构：
工作分解结构：是以可交付成果为导向的工作层级分解， 工作分解结构定义项目的全部范  
...
已经识别的风险，风险发生还是风险，不是问题。针对未知-未知风险，发生才是问题。已经识别的风险，记录在风险登记册中，风险发生，还是要查风险登记册的应对办法，不会去看问题日志

工作包层是WBS中的最低层，便于资源分配和成本、时间估算。敏捷项目的工作包层是用户故事。在本题中，工作包层包含四个子项目，所以不会用来创建活动清单。活动清单将从工作包层创建，为每个子项目创建的每个WBS创建活动清单。
...
PTA （point of total assumption） 最高限价  
